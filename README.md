...
The mod significantly shrinks the window during which a totem might fail to activate due to server-client desynchronization. By sending your hotbar change packet instantly, the server is informed of your totem's position much sooner.
  ...
  - High Ping (e.g., 150ms round trip): While ping remains the dominant factor, the mod still offers a tangible reduction of the client-side delay. The "ghost totem window" can be reduced by approximately 25%, cutting a 100ms window (25ms average client delay + 75ms ping) down to 75ms (ping alone). lol
